...
export async function POST(req) {
  try {
    ...
    // Kart girmeyen (Deneme / Trial) kullanıcılar müşterilerinden online ödeme alamaz
    if (!hasFeature(tenant.plan, "onlinePayment")) {
      return NextResponse.json({ error: "Online ödeme alabilmek için lütfen Fiibi aboneliğinizi başlatın (Kartınızı tanımlayın)." }, { status: 403 });
    }
