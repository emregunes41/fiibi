      ...
      <div style={{ display: "flex", gap: "16px", marginBottom: "24px", flexWrap: "wrap" }}>
        <div style={{ position: "relative", flex: 1, minWidth: "250px" }}>
          ...
          <input 
            ...
            style={{ ...inp, paddingLeft: "36px", background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.08)", borderRadius: "4px" }}
          />
        </div>
          ...
          {["ALL", "PENDING", "CONFIRMED", "SHIPPED", "COMPLETED", "CANCELED"].map(st => (
            <button key={st} onClick={() => setFilterStatus(st)}
              style={{
                ...
                color: filterStatus === st ? "#000" : "rgba(0,0,0,0.5)",
                borderColor: filterStatus === st ? "#fff" : "rgba(0,0,0,0.1)",
              }}
            >
              {st === "ALL" ? "Tümü" : 
               st === "PENDING" ? "Bekleyen" : 
          ...
          {filteredOrders.map(order => {
            ...
            return (
              <div key={order.id} style={{ 
                ...
                <div style={{ padding: "16px 24px", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                    ...
                    <button onClick={() => setDetailModal({ isOpen: true, data: { ...order, productList, hasPhysical } })} style={{
                      padding: "8px 16px", background: "rgba(0,0,0,0.05)", border: "none", color: "#1a1a1a",
                ...
                <div style={{ padding: "12px 24px", background: "rgba(0,0,0,0.2)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                   ...
                   <div style={{ display: "flex", gap: "6px" }}>
                      ...
                      {order.status === "CONFIRMED" && hasPhysical && (
                        <button onClick={() => setShippingModal({ isOpen: true, orderId: order.id, trackingUrl: "" })} style={{ padding: "6px 12px", background: "#3b82f6", border: "none", color: "#1a1a1a", fontSize: "12px", fontWeight: 700, borderRadius: "4px", cursor: "pointer" }}>
                          Kargoya Ver
                        </button>
                      )}
                      {order.status === "CONFIRMED" && !hasPhysical && (
                        <button onClick={() => handleStatusChange(order.id, "COMPLETED")} style={{ padding: "6px 12px", background: "#10b981", border: "none", color: "#1a1a1a", fontSize: "12px", fontWeight: 700, borderRadius: "4px", cursor: "pointer" }}>
                          Tamamlandı İşaretle
                        </button>
                      )}
        ...
        {detailModal.isOpen && (
          <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyItems: "center" }}>
            ...
            <div style={{
              ...
              border: "1px solid rgba(0,0,0,0.1)", borderRadius: "12px", overflow: "hidden",
            }}>
              <div style={{ padding: "20px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => setDetailModal({ isOpen: false, data: null })} style={{ background: "none", border: "none", color: "#1a1a1a", cursor: "pointer" }}><X size={20} /></button>
              </div>
              ...
              <div style={{ padding: "24px", maxHeight: "70vh", overflowY: "auto" }}>
                ...
                <div style={{ marginBottom: "24px" }}>
                  ...
                  <div style={{ background: "rgba(0,0,0,0.02)", padding: "16px", borderRadius: "8px", border: "1px solid rgba(0,0,0,0.05)", display: "flex", flexDirection: "column", gap: "12px" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "10px", fontSize: "14px" }}><Mail size={16} color="rgba(0,0,0,0.4)" /> {detailModal.data.brideEmail}</div>
                  </div>
                </div>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {detailModal.data.productList.map((item, idx) => (
                      <div key={idx} style={{ 
                        ...
                        padding: "16px", background: "rgba(0,0,0,0.03)", borderRadius: "8px", border: "1px solid rgba(0,0,0,0.05)"
                      }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 9999, display: "flex", alignItems: "center", justifyItems: "center" }}>
            ...
            <div style={{
              ...
              <div style={{ padding: "20px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => setShippingModal({ isOpen: false, orderId: null, trackingUrl: "" })} style={{ background: "none", border: "none", color: "#1a1a1a", cursor: "pointer" }}><X size={20} /></button>
              </div>
              ...
              <div style={{ padding: "24px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "flex-end", gap: "12px" }}>
                  <button onClick={() => setShippingModal({ isOpen: false, orderId: null, trackingUrl: "" })} style={{ padding: "10px 16px", background: "transparent", border: "1px solid rgba(0,0,0,0.1)", color: "#1a1a1a", borderRadius: "6px", cursor: "pointer", fontWeight: 600 }}>
                    İptal
                  </button>
                  <button onClick={handleShippingSubmit} style={{ padding: "10px 16px", background: "#3b82f6", border: "none", color: "#1a1a1a", borderRadius: "6px", cursor: "pointer", fontWeight: 600, display: "flex", alignItems: "center", gap: "6px" }}>
