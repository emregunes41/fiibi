  ...
  return (
    <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 20px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32, flexWrap: "wrap", gap: 16 }}>
        ...
        <button onClick={() => openForm()} style={{ display: "flex", alignItems: "center", gap: 8, background: "var(--text)", color: "var(--bg)", border: "none", padding: "12px 20px", borderRadius: 0, fontWeight: 700, cursor: "pointer" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 24 }}>
        {events.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 0, gridColumn: "1 / -1" }}>
            ...
            <button onClick={() => openForm()} style={{ background: "rgba(255,255,255,0.1)", color: "var(--text)", border: "none", padding: "10px 16px", borderRadius: 0, fontWeight: 600, cursor: "pointer" }}>
            ...
            return (
              <div key={ev.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 0, overflow: "hidden", position: "relative" }}>
                ...
                <div style={{ padding: 20, background: "rgba(0,0,0,0.15)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 12 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 700 }}><span style={{ color: "var(--text)" }}>{registrationsCount}</span> <span style={{ color: "rgba(255,255,255,0.3)" }}>/ {ev.maxParticipants} Kişi</span></div>
                    </div>
                ...
                <div style={{ display: "flex", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                  <button onClick={() => { setSelectedEvent(ev); setParticipantsOpen(true); }} style={{ flex: 1, padding: 16, background: "none", border: "none", borderRight: "1px solid rgba(255,255,255,0.04)", color: "var(--text)", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
      ...
      {formOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", justifyContent: "center", alignItems: "center", padding: 20 }}>
          <div style={{ background: "var(--bg)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 0, width: "100%", maxWidth: 600, maxHeight: "90vh", overflowY: "auto" }}>
            <div style={{ padding: "24px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800 }}>{editingId ? "Etkinliği Düzenle" : "Yeni Etkinlik Aç"}</h2>
              <button onClick={() => setFormOpen(false)} style={{ background: "none", border: "none", color: "var(--text)", cursor: "pointer" }}><X size={24} /></button>
            </div>
            <form onSubmit={handleSave} style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} placeholder="Örn: Cumartesi Sabahı Canlı Pilates Grubu" />
              </div>

              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.7)", marginBottom: 8 }}>Kısa Açıklama</label>
                <textarea required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14, minHeight: 80 }} placeholder="Katılımcılara etkinliğin ne hakkında olduğunu anlatın..." />
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  ...
                  <input required type="datetime-local" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} />
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.7)", marginBottom: 8 }}>Süre (Dakika)</label>
                  <input required type="number" min="15" value={formData.durationMinutes} onChange={e => setFormData({...formData, durationMinutes: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} />
                </div>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  ...
                  <input required type="number" min="0" value={formData.price} onChange={e => setFormData({...formData, price: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} placeholder="Ücretsiz ise 0 yazın" />
                ...
                <div>
                  ...
                  <input required type="number" min="1" value={formData.maxParticipants} onChange={e => setFormData({...formData, maxParticipants: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} />
                </div>
              </div>
              ...
              <div style={{ padding: 16, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 0 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <input type="text" value={formData.location} onChange={e => setFormData({...formData, location: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} placeholder="Fiziksel Adres / Salon İsmi" />
                  <input type="url" value={formData.meetingLink} onChange={e => setFormData({...formData, meetingLink: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "var(--text)", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} placeholder="Zoom / Meet Görüşme Linki (varsa)" />
                </div>
              </div>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", background: "rgba(255,255,255,0.02)", padding: '12px 16px', borderRadius: 0, border: "1px solid rgba(255,255,255,0.05)" }}>
                <input type="checkbox" checked={formData.isActive} onChange={e => setFormData({...formData, isActive: e.target.checked})} style={{ width: 18, height: 18, accentColor: "var(--text)" }} />
                <span style={{ fontSize: 14, fontWeight: 600 }}>Etkinliği Satışa/Kayıda Aç</span>
              </label>

              <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
                <button type="button" onClick={() => setFormOpen(false)} style={{ flex: 1, padding: "14px", background: "rgba(255,255,255,0.05)", border: "none", color: "var(--text)", fontWeight: 700, borderRadius: 0, cursor: "pointer" }}>İptal</button>
                <button type="submit" style={{ flex: 1, padding: "14px", background: "var(--text)", border: "none", color: "var(--bg)", fontWeight: 700, borderRadius: 0, cursor: "pointer" }}>Kaydet</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {participantsOpen && selectedEvent && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", justifyContent: "center", alignItems: "center", padding: 20 }}>
          <div style={{ background: "var(--bg)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 0, width: "100%", maxWidth: 800, maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "24px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
              ...
              <button onClick={() => setParticipantsOpen(false)} style={{ background: "none", border: "none", color: "var(--text)", cursor: "pointer" }}><X size={24} /></button>
            </div>
