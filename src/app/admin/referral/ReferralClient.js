  ...
  return (
    <div style={{ color: "#1a1a1a" }}>
      ...
      <div style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.08)", padding: "28px 24px", marginBottom: 16 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(0,0,0,0.65)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 12 }}>
          Referans Kodunuz
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <button onClick={() => handleCopy(code)} style={{
            ...
            color: "rgba(0,0,0,0.65)", padding: "8px 14px", cursor: "pointer", fontSize: 12,
      ...
      <div style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.06)", padding: "16px 20px", marginBottom: 24 }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(0,0,0,0.65)", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 8 }}>
          Paylaşım Linki
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <code style={{ fontSize: 12, color: "rgba(0,0,0,0.65)", wordBreak: "break-all", flex: 1 }}>{referralLink}</code>
          <button onClick={() => handleCopy(referralLink)} style={{
            background: "rgba(0,0,0,0.06)", border: "1px solid rgba(0,0,0,0.1)",
            color: "rgba(0,0,0,0.65)", padding: "6px 10px", cursor: "pointer", fontSize: 11,
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12, marginBottom: 32 }}>
        {[
          ...
          <div key={i} style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.06)", padding: "20px 16px" }}>
            ...
            <div style={{ fontSize: 12, color: "rgba(0,0,0,0.65)", lineHeight: 1.5 }}>{s.desc}</div>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 32 }}>
        <div style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.08)", padding: "16px 20px", flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(0,0,0,0.65)", textTransform: "uppercase", marginBottom: 6 }}>Toplam Davet</div>
          <div style={{ fontSize: 28, fontWeight: 900 }}>{tenant?.referralCount || 0}</div>
        </div>
        <div style={{ background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.08)", padding: "16px 20px", flex: 1 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "rgba(0,0,0,0.65)", textTransform: "uppercase", marginBottom: 6 }}>Kazanılan Süre</div>
          <div style={{ fontSize: 28, fontWeight: 900 }}>{(tenant?.referralCount || 0) * 30} <span style={{ fontSize: 14, color: "rgba(0,0,0,0.65)" }}>gün</span></div>
        </div>
      </div>
      ...
      {referrals.length > 0 && (
        <div style={{ background: "rgba(0,0,0,0.02)", border: "1px solid rgba(0,0,0,0.06)" }}>
          <div style={{ padding: "14px 20px", borderBottom: "1px solid rgba(0,0,0,0.06)", fontSize: 12, fontWeight: 700, color: "rgba(0,0,0,0.65)", textTransform: "uppercase", display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {referrals.map((r, i) => (
            <div key={i} style={{ padding: "14px 20px", borderBottom: i < referrals.length - 1 ? "1px solid rgba(0,0,0,0.04)" : "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <div style={{ fontSize: 11, color: "rgba(0,0,0,0.65)" }}>{r.slug}.{domain}</div>
              </div>
