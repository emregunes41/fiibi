    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 24 }}>
        {posts.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", background: "rgba(255,255,255,0.02)", border: "1px dashed rgba(255,255,255,0.1)", borderRadius: 0, gridColumn: "1 / -1" }}>
            <FileText size={40} style={{ color: "rgba(255,255,255,0.2)", margin: "0 auto 16px" }} />
            <p style={{ color: "rgba(255,255,255,0.5)", marginBottom: 16 }}>Sisteminizde hiç blog yazısı bulunmuyor.</p>
          ...
          posts.map(post => (
            <div key={post.id} style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 0, overflow: "hidden", position: "relative", display: "flex", flexDirection: "column" }}>
              ...
              {post.imageUrl ? (
                ...
                <div style={{ width: "100%", height: 160, background: "rgba(255,255,255,0.02)", display: "flex", alignItems: "center", justifyContent: "center" }}>
              ...
              <div style={{ padding: 20, flex: 1 }}>
                ...
                {post.excerpt && <p style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", margin: 0, display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{post.excerpt}</p>}
              </div>

              <div style={{ display: "flex", borderTop: "1px solid rgba(255,255,255,0.04)" }}>
                <a href={`/blog/${post.slug}`} target="_blank" style={{ flex: 1, padding: 16, background: "none", border: "none", borderRight: "1px solid rgba(255,255,255,0.04)", color: "#fff", fontWeight: 700, fontSize: 13, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", textDecoration: "none" }}>
                  Görüntüle
                </a>
                <button onClick={() => openForm(post)} style={{ padding: 16, background: "none", border: "none", borderRight: "1px solid rgba(255,255,255,0.04)", color: "rgba(255,255,255,0.6)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <Edit2 size={16} />
                </button>
      ...
      {formOpen && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", justifyContent: "center", alignItems: "center", padding: 20 }}>
          <div className="admin-modal-content" style={{ border: "1px solid rgba(255,255,255,0.1)", borderRadius: 0, width: "100%", maxWidth: 800, maxHeight: "90vh", overflowY: "auto", background: "#111" }}>
            <div style={{ padding: "24px", borderBottom: "1px solid rgba(255,255,255,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center", position: "sticky", top: 0, background: "#111", zIndex: 10 }}>
            ...
            <form onSubmit={handleSave} style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                ...
                {formData.imageUrl ? (
                  <div style={{ position: "relative" }}>
                    <img src={formData.imageUrl} alt="Kapak" style={{ width: "100%", height: 240, objectFit: "cover", borderRadius: 0, border: "1px solid rgba(255,255,255,0.1)" }} />
              ...
              <div>
                ...
                <input required type="text" value={formData.title} onChange={e => setFormData({...formData, title: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "12px 16px", borderRadius: 0, fontSize: 14 }} placeholder="Örn: 2026 Düğün Fotoğrafçılığı Trendleri" />
              </div>

              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.7)", marginBottom: 8 }}>Özet (Opsiyonel)</label>
                <textarea value={formData.excerpt} onChange={e => setFormData({...formData, excerpt: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "12px 16px", borderRadius: 0, fontSize: 14, minHeight: 60 }} placeholder="Yazının kısa bir özeti..." />
              </div>

              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(255,255,255,0.7)", marginBottom: 8 }}>İçerik</label>
                <textarea required value={formData.content} onChange={e => setFormData({...formData, content: e.target.value})} style={{ width: "100%", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "#fff", padding: "16px", borderRadius: 0, fontSize: 15, minHeight: 300, lineHeight: 1.6 }} placeholder="Yazı içeriğini buraya girin (HTML destekler)..." />
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", marginTop: 6 }}>Basit HTML etiketleri (&lt;b&gt;, &lt;i&gt;, &lt;h2&gt;, vb.) kullanabilirsiniz.</p>
              </div>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer", background: "rgba(255,255,255,0.02)", padding: '12px 16px', borderRadius: 0, border: "1px solid rgba(255,255,255,0.05)" }}>
                <input type="checkbox" checked={formData.isPublished} onChange={e => setFormData({...formData, isPublished: e.target.checked})} style={{ width: 18, height: 18, accentColor: "#fff" }} />
