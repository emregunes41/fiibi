...
export default function DashboardInteractiveLists({ 
  ...
  const getDaysLeftInfo = (targetDate) => {
    ...
    return { text: `${diffDays} gün kaldı`, color: "#1a1a1a" };
  };
          ...
          {upcomingDeliveries.map((res) => {
            ...
            return (
              <div 
                ...
                style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 0, background: isLate ? "rgba(239,68,68,0.05)" : "rgba(0,0,0,0.04)", border: isLate ? "1px solid rgba(239,68,68,0.2)" : "1px solid rgba(0,0,0,0.06)" }}
              >
            ...
            {upcomingShoots.map((res) => {
              ...
              return (
                <div 
                  key={res.id} 
                  onClick={() => setDetailModal({ isOpen: true, data: res })}
                  style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 10px", borderRadius: 0, background: "rgba(0,0,0,0.04)", border: "1px solid rgba(0,0,0,0.06)" }}
                >
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
            {recentReservations.map((res) => (
              <div 
                key={res.id} 
                onClick={() => setDetailModal({ isOpen: true, data: res })}
                style={{ cursor: "pointer", display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 8px", borderRadius: 0, background: "rgba(0,0,0,0.04)" }}
              >
                <div>
