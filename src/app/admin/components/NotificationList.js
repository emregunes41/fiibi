...
export default function NotificationList({ notifications }) {
  ...
  return (
    <div style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 0, overflow: "hidden", marginBottom: "3rem" }}>
      <div style={{ padding: "1.5rem 2rem", borderBottom: "1px solid rgba(255,255,255,0.1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        ...
        {unread.length > 0 && (
          <button 
            ...
            disabled={isClearing}
            style={{ background: "transparent", border: "1px solid rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.6)", padding: "0.4rem 1rem", borderRadius: 0, fontSize: "0.75rem", fontWeight: 800, cursor: "pointer", transition: "all 0.2s" }}
      ...
      <div style={{ padding: "1rem" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
          {unread.map(notif => (
            <div key={notif.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "1rem 1.5rem", background: "rgba(255,255,255,0.06)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 0, gap: "1rem" }}>
              <div>
          ...
          {read.map(notif => (
            <div key={notif.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", padding: "1rem 1.5rem", background: "transparent", border: "1px solid rgba(255,255,255,0.05)", borderRadius: 0, gap: "1rem", opacity: 0.6 }}>
              <div>
