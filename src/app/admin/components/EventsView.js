      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 24 }}>
        {events.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", background: "rgba(0,0,0,0.02)", border: "1px dashed rgba(0,0,0,0.1)", borderRadius: 0, gridColumn: "1 / -1" }}>
            <Ticket size={40} style={{ color: "rgba(0,0,0,0.65)", margin: "0 auto 16px" }} />
            <p style={{ color: "rgba(0,0,0,0.65)", marginBottom: 16 }}>Sisteminizde hiç etkinlik bulunmuyor.</p>
            ...
            return (
              <div key={ev.id} style={{ background: "rgba(0,0,0,0.03)", border: "1px solid rgba(0,0,0,0.08)", borderRadius: 0, overflow: "hidden", position: "relative" }}>
                ...
                <div style={{ padding: 20, borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                  ...
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", fontSize: 11, fontWeight: 600, color: "rgba(0,0,0,0.65)" }}>
                ...
                <div style={{ padding: 20, background: "rgba(0,0,0,0.15)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 12 }}>
                    <div>
                      <div style={{ fontSize: 11, color: "rgba(0,0,0,0.65)", fontWeight: 600, marginBottom: 4 }}>Kontenjan Durumu</div>
                      <div style={{ fontSize: 14, fontWeight: 700 }}><span style={{ color: "#1a1a1a" }}>{registrationsCount}</span> <span style={{ color: "rgba(0,0,0,0.65)" }}>/ {ev.maxParticipants} Kişi</span></div>
                    </div>
                ...
                <div style={{ display: "flex", borderTop: "1px solid rgba(0,0,0,0.04)" }}>
                  ...
                  <button onClick={() => openForm(ev)} style={{ padding: 16, background: "none", border: "none", borderRight: "1px solid rgba(0,0,0,0.04)", color: "rgba(0,0,0,0.6)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <Edit2 size={16} />
                  </button>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 100, display: "flex", justifyContent: "center", alignItems: "center", padding: 20 }}>
          <div className="admin-modal-content" style={{ border: "1px solid rgba(0,0,0,0.1)", borderRadius: 0, width: "100%", maxWidth: 600, maxHeight: "90vh", overflowY: "auto" }}>
            ...
            <form onSubmit={handleSave} style={{ padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
              ...
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(0,0,0,0.7)", marginBottom: 8 }}>Etkinlik Başlığı</label>
              ...
              <div>
                <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(0,0,0,0.7)", marginBottom: 8 }}>Kısa Açıklama</label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(0,0,0,0.7)", marginBottom: 8 }}>Tarih ve Saat</label>
                ...
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(0,0,0,0.7)", marginBottom: 8 }}>Süre (Dakika)</label>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(0,0,0,0.7)", marginBottom: 8 }}>Kişi Başı Ücret (₺)</label>
                  ...
                  <p style={{ fontSize: 11, color: "rgba(0,0,0,0.65)", marginTop: 6 }}>Ücretsiz yapmak için 0 yazın.</p>
                </div>
                <div>
                  <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "rgba(0,0,0,0.7)", marginBottom: 8 }}>Maksimum Kontenjan</label>
              ...
              <div style={{ background: formData.showInDiscovery ? "rgba(59,130,246,0.08)" : "rgba(0,0,0,0.02)", border: `1px solid ${formData.showInDiscovery ? "rgba(59,130,246,0.25)" : "rgba(0,0,0,0.05)"}`, borderRadius: 0, padding: "14px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, transition: "all 0.2s" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 700, color: formData.showInDiscovery ? "#fff" : "rgba(0,0,0,0.6)" }}>Keşfet'te Göster</div>
                    <div style={{ fontSize: 11, color: "rgba(0,0,0,0.65)" }}>Bu etkinlik fiibi.co ana sayfasında listelensin</div>
                  </div>
                </div>
                {canUseDiscovery ? (
          ...
          <div className="admin-modal-content" style={{ border: "1px solid rgba(0,0,0,0.1)", borderRadius: 0, width: "100%", maxWidth: 800, maxHeight: "90vh", display: "flex", flexDirection: "column" }}>
            <div style={{ padding: "24px", borderBottom: "1px solid rgba(0,0,0,0.05)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div>
                ...
                <p style={{ margin: 0, fontSize: 13, color: "rgba(0,0,0,0.65)" }}>Katılımcı Listesi ({selectedEvent.registrations?.length || 0} / {selectedEvent.maxParticipants})</p>
              </div>
            ...
            <div style={{ padding: 24, overflowY: "auto", flex: 1 }}>
              {selectedEvent.registrations?.length === 0 ? (
                <p style={{ color: "rgba(0,0,0,0.65)", textAlign: "center", padding: 40 }}>Henüz kayıtlı katılımcı yok.</p>
              ) : (
                <div style={{ overflowX: "auto" }}>
                  <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: 13 }}>
                    <thead>
                      <tr style={{ background: "rgba(0,0,0,0.05)" }}>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600 }}>İsim Soyisim</th>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600 }}>Telefon</th>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600 }}>E-Posta</th>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600 }}>Sosyal Medya</th>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600 }}>Kayıt Tarihi</th>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600 }}>Ödeme</th>
                        <th style={{ padding: 12, borderBottom: "1px solid rgba(0,0,0,0.1)", color: "rgba(0,0,0,0.6)", fontWeight: 600, width: 50 }}></th>
                      </tr>
                    </thead>
                    <tbody>
                      {selectedEvent.registrations.map(reg => (
                        <tr key={reg.id} style={{ borderBottom: "1px solid rgba(0,0,0,0.04)" }}>
                          ...
                          <td style={{ padding: 12, color: "rgba(0,0,0,0.7)" }}>{reg.phone}</td>
                          <td style={{ padding: 12, color: "rgba(0,0,0,0.7)" }}>{reg.email}</td>
                          <td style={{ padding: 12, color: "rgba(0,0,0,0.7)" }}>{reg.socialMedia || "—"}</td>
                          <td style={{ padding: 12, color: "rgba(0,0,0,0.65)" }} suppressHydrationWarning>{new Date(reg.createdAt).toLocaleDateString("tr-TR")}</td>
                          <td style={{ padding: 12 }}>
                            {reg.paymentStatus === "PAID" || selectedEvent.price === "0" ? (
                              <span style={{ fontSize: 10, fontWeight: 800, background: "rgba(74, 222, 128, 0.15)", color: "#4ade80", padding: "4px 8px", borderRadius: 0 }}>ÖDENDİ</span>
                               ...
                               <span style={{ fontSize: 10, fontWeight: 800, background: "rgba(0, 0, 0, 0.1)", color: "rgba(0,0,0,0.7)", padding: "4px 8px", borderRadius: 0 }}>NAKİT / BEKLİYOR</span>
                            )}
                          </td>
                          <td style={{ padding: 12 }}>
